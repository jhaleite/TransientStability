...
arquivo = open('C:\\Users\\Joao Henrique Leite\\Biblioteca de Programas em Python\\TrabalhoEstabilidade14barras\\IEEE14.txt','r')
...
tfoult =  0.090      # Tempo de extição da falta (em segundo)
...
xPOS = solve_ivp(fun=fun1,t_span=[tfoult,tAfter],y0=x1,max_step=maxstep)
...
for i in range(ng):
        plt.figure(1)
        plt.plot(np.hstack((tf,tp)),np.hstack((Wf[i],Wp[i])), label=('Gerador ',i+1))      
        plt.title(' Grafico de w no tempo ')
        plt.xlabel(' tempo (s)')
        plt.ylabel('Velocidade ângular (rad/s)')
        plt.legend()
        plt.grid(True)
        plt.figure(2)
        plt.plot(np.hstack((tf,tp)),np.hstack((Df[i],Dp[i])),label=('Gerador ',i+1))
        plt.title(' Grafico das angulos internos dos geradores no tempo ')
        plt.xlabel(' tempo (s)')
